...
class WidgetGroupTest extends TestCase
{
	...
	public function testItCanDisplayWidgets()
	{
		...
		$this->assertEquals('Slider was executed with $slides = 5 foo: bar' . '<div id="flinnt-widget-container-2" style="display:inline" class="flinnt-widget-container">Placeholder here!' . '<script type="text/javascript">' . 'var widgetTimer2 = setInterval(function() {' . 'if (window.$) {' . "$('#flinnt-widget-container-2').load('" . $this->ajaxUrl('Slider', [], 2) . "');" . 'clearInterval(widgetTimer2);' . '}' . '}, 100);' . '</script>' . '</div>', $output);
	}
	...
	public function testItCanSetAndResetPosition()
	{
		...
		$this->assertEquals('Slider was executed with $slides = 6 foo: bar' . 'Slider was executed with $slides = 5 foo: Taylor', $output);
	...
	public function testMultipleWidgetGroupsCanExistTogether()
	{
		...
		$this->assertEquals('Slider was executed with $slides = 10 foo: bar' . 'Slider was executed with $slides = 15 foo: bar', $output2);
	...
	public function testSeparator()
	{
		...
		$this->assertEquals('Slider was executed with $slides = 5 foo: bar' . '<hr>' . '<div id="flinnt-widget-container-2" style="display:inline" class="flinnt-widget-container">Placeholder here!' . '<script type="text/javascript">' . 'var widgetTimer2 = setInterval(function() {' . 'if (window.$) {' . "$('#flinnt-widget-container-2').load('" . $this->ajaxUrl('Slider', [], 2) . "');" . 'clearInterval(widgetTimer2);' . '}' . '}, 100);' . '</script>' . '</div>', $output);
	}
