...
use Flinnt\Core\View\Widgets\Test\Dummies\TestCachedWidget;
...
class WidgetFactoryTest extends TestCase
{
	...
	{
		$this->setExpectedException('\Flinnt\Core\View\Widgets\Misc\InvalidWidgetClassException');
	...
	{
		$output = $this->factory->run('\Flinnt\Core\View\Widgets\Test\Dummies\TestDefaultSlider');
	...
	public function testItCanRunReloadableWidgets()
	{
		...
		$this->assertEquals('<div id="flinnt-widget-container-1" style="display:inline" class="arrilot-widget-container">Feed was executed with $slides = 6' . '<script type="text/javascript">' . 'setTimeout( function() {' . 'var widgetTimer1 = setInterval(function() {' . 'if (window.$) {' . "$('#flinnt-widget-container-1').load('" . $this->ajaxUrl('TestRepeatableFeed') . "');" . 'clearInterval(widgetTimer1);' . '}' . '}, 100);' . '}, 10000)' . '</script>' . '</div>', $output);
	}
	...
	public function testWidgetContainerCanBeCustomized()
	{
		...
		$this->assertEquals('<p id="flinnt-widget-container-1" data-id="123">Dummy Content' . '<script type="text/javascript">' . 'setTimeout( function() {' . 'var widgetTimer1 = setInterval(function() {' . 'if (window.$) {' . "$('#flinnt-widget-container-1').load('" . $this->ajaxUrl('TestWidgetWithCustomContainer') . "');" . 'clearInterval(widgetTimer1);' . '}' . '}, 100);' . '}, 10000)' . '</script>' . '</p>', $output);
	}
	...
	public function testItCanCacheWidgets()
	{
		...
		$key = 'flinnt.widgets.' . serialize(['testCachedWidget', ['foo' => 'bar']]);
